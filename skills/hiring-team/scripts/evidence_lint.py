#!/usr/bin/env python3
"""Evidence gate and citation lint for the hiring-team skill.
...
Two modes:
  ...
  actually given, and that unsupported claims stay under the role's ceiling:
      ...
      python scripts/evidence_lint.py roles/evidence-assessor.md \\
          --evidence slices/evidence-assessor.md
...
CEILINGS = {
    "evidence-assessor": 0.25,
    "cost-capacity-analyst": 0.25,
    "role-architect": 0.35,
    "team-integrator": 0.40,
    "historian": 0.35,
    "talent-lead": 0.20,
}
...
STRONG_TYPES = ("demonstrated", "reference", "internal-data")
...
def gate(items, path: Path, stage=None):
    ...
    print(f"  strong (demonstrated + reference + internal-data): {len(strong)} items "
    ...
    if n_sources < MIN_STRONG_SOURCES:
        stops.append(
            f"only {n_sources} independent demonstrated/reference/internal-data source(s); "
    ...
    if types.get("demonstrated", 0) == 0:
        msg = ("zero demonstrated items -- no actual work has been examined; "
               "the analysis would run on claims and interview polish")
        (stops if stage == "candidate" else marginal).append(msg)

    if types.get("internal-data", 0) == 0:
        msg = ("zero internal-data items -- nobody has measured what the gap "
               "costs; 'we're drowning' is a feeling, not a finding")
        (stops if stage == "role" else marginal).append(msg)

    if types.get("reference", 0) == 0 and stage == "candidate":
        marginal.append("zero reference items -- nobody who watched them work has been asked")

    print()
    print("  These conditions need your judgement and are not checked here:")
    print("    - does the decision hinge on a capability that no demonstrated item")
    print("      actually shows? (a work trial about the wrong thing still counts here)")
    print("    - is every reference from the candidate's own curated list?")
    print("    - when comparing finalists, was the same evidence collected with the")
    print("      same effort for each? (a trial vs. a warm referral is not a comparison)")
    print("    - has any protected-characteristic material been stripped on entry?")
    print()
    ...
    if stops:
        ...
        print("  Cheapest fixes are usually: a paid work sample, two structured")
        print("  reference calls, or a week measuring the gap.")
        print("  See references/evidence-protocol.md. Do not spawn any role.")
        return False

    if marginal:
        print("GATE: MARGINAL PASS -- verdict is capped at 'trial'")
        for m in marginal:
            print(f"  - {m}")
        print()
...
def lint(path: Path, evidence_ids, role, ceiling):
    ...
    if role == "talent-lead" and counts["assumption"]:
        notes.append(
            f"{counts['assumption']} [ASSUMPTION] line(s) in synthesis -- the "
            "Talent Lead may only carry claims forward, never introduce them"
        )
    if total == 0:
...
def main() -> int:
    ...
    ap.add_argument("--stage", choices=["role", "candidate"],
                    help="role: 'should this role exist?' (fails on zero internal-data); "
                         "candidate: deciding on people (fails on zero demonstrated)")
    ...
    if args.gate or not args.files:
        if items is None:
            ...
            return 2
        return 0 if gate(items, Path(args.evidence), args.stage) else 1
