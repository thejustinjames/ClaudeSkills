...
flags load-bearing assumptions with no test attached, and applies the rule that
a load-bearing assumption at priority >= 16 with status 'untested' forces the
verdict to 'test' rather than 'proceed'.
...
Exit codes: 0 unconstrained, 1 verdict constrained to 'test', 2 ledger unusable.
"""
...
def main() -> int:
    ...
    ap.add_argument("--threshold", type=int, default=16, help="priority forcing a test verdict")
    args = ap.parse_args()
    ...
    for r in rows:
        ...
        print(
            ...
            f"{text}{'   <-- forces test' if forcing else ''}"
        )
    ...
    print()
    if no_test:
        print("Load-bearing assumptions with no test attached:")
        for r in no_test:
            print(f"  {r['id']}: {r['assumption'][:70]}")
        print("  These are what kills the decision six months from now. Write a test")
        print("  for each, or set status to 'accepted' with a reason in the memo.")
        print()

    if forcing:
        print(f"VERDICT CONSTRAINED TO 'TEST' -- {len(forcing)} load-bearing assumption(s) "
              f"at priority >= {args.threshold}, untested:")
        for r in forcing:
            print(f"  {r['id']} (priority {r['priority']}): {r['assumption'][:70]}")
        print("  There is no exception for a strong narrative. Run the test first.")
        print("  For an either/or decision, apply this per option -- see memo-format.md.")
        return 1
