---
name: decision-team
description: Run a rigorous multi-role strategy analysis on a consequential decision using isolated role agents, enforced evidence citation, adversarial red-teaming, and a falsifiable test list. Use whenever someone is weighing a real decision — build or buy, raise prices, enter a market, hire, pivot, kill a product, pick a vendor, sign a contract, take the funding — or asks for a strategy review, decision memo, business case, pre-mortem, go/no-go, or "help me think through" a big call. Also use when they want assumptions ranked, uncertainty turned into experiments, kill criteria defined, or a red-team critique of a plan they already have. Trigger even when the request is casual ("should we...", "I'm torn between...", "is this worth doing") as long as the stakes are real.
---
...
enough to work with. Read `references/evidence-protocol.md` for the full rules,
but the shape is:
...
Interview the user for the decision statement; do not fill it from inference.
A vague statement poisons everything downstream, because each role will
silently resolve the ambiguity in a different direction. It needs: the specific
move, the alternative it is being weighed against, the date, the desired
outcome in observable terms, the resources actually available, and what would
count as success at six months. Mark anything the user cannot supply
`[NOT SUPPLIED]` rather than filling it in — those gaps are the first finding.
...
`references/evidence-protocol.md`): one block per item, each with an `E<n>` id,
a source, and a type. Evidence comes from the user, from files, or from tools
with retrievable citations. Never write an evidence item from your own priors —
if you researched it with web search, it is `secondary` and carries a URL.
...
The gate counts *independent sources*, not blocks, so splitting one interview
into four well-scoped items does not inflate it. Pass `--hinges-on-demand` when
the decision turns on whether anyone wants the thing. The script prints the
conditions it cannot check — apply those yourself, particularly the buyer/user
split: in a B2B2C decision, interviews with the people who pay say nothing
about demand from the people who would use it.
...
Assign each evidence item to the roles that should see it and record the
mapping in `slices.md`. Write each slice out as a standalone file under
`slices/` containing the full text of its items — a role should never be handed
the whole evidence file with instructions to ignore parts of it.

`references/roles.md` gives the default mapping and the rule for when the
evidence is too thin to slice five ways. Slicing is what buys independence, and
it costs something: a role may miss what it needed. That is handled in step 4
rather than by giving everyone everything.
...
Role briefs are in `references/roles.md`. The five are Market Analyst, Customer
Strategist, Operator, Finance Lead, and Historian. (Red team and synthesis run
later and separately — that ordering is the point.)

Every role deliverable ends with a section titled **"Evidence I was not given but
need"**, listing what it would have wanted. Collect those. They are often the
most useful output of the whole step, because they map the information gaps
...
Two script outputs are **notes, not failures**: a high `[UNKNOWN]` share means
the slice was too thin for that role, and that belongs in the memo's
limitations rather than in a redo. Re-running an agent to reword an honest gap
costs a round trip and changes no finding.

### 6. Map disagreement mechanically

Do not ask the roles to disagree. Manufactured disagreement is theatre; it
reads as rigour and contains no information. Instead, extract every claim that
bears on the same assumption and find where their polarity conflicts. Those are
real disagreements — they arose from different evidence.
...
**If the roles agree on everything, that is a warning, not a green light.** It
most likely means they shared a prior or the evidence slices were too similar.
Note it explicitly, and instruct the red team to attack the consensus directly.
...
enforced. The memo is then rendered from it in readable prose and is **not**
linted; a one-page memo with a citation tag on every line is unreadable, and an
unreadable memo does not get read. Format in `references/memo-format.md`.

Then run the red team per
`references/redteam-protocol.md`: three independent refuters, distinct lenses,
each given the memo and the raw evidence file but **not** the role deliverables
or any reasoning trace, each asked to break the decision rather than review it.
...
or duration, the cost, and — written *before* the test runs and timestamped —
the pass threshold, the fail threshold, and the band in between that means
modify. Format in `references/memo-format.md`.
...
2. **The decision memo** — one page, per `references/memo-format.md`, with the
...
- `references/evidence-protocol.md` — evidence format, types, tagging, the gate,
  lint thresholds. Read before step 2.
- `references/roles.md` — the six role briefs, evidence slices, output
  contracts. Read before step 4.
- `references/redteam-protocol.md` — refuter lenses, survivability rules, kill
  conditions. Read before step 8.
- `references/memo-format.md` — memo template, assumption ledger schema, test
  card format. Read before step 8.
- `assets/examples/worked-example.md` — a short end-to-end example, including
  one that correctly fails the evidence gate.
